...
export function UpdatesPage() {
  ...
  return (
    <div>
      ...
      <PageSection>
        {/* Filters */}
        <Stagger stagger={0.04} style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 32 }}>
          {u.categories.map((cat) => (
            <StaggerItem key={cat.id} y={8}>
              <motion.button
                onClick={() => setFilter(cat.id)}
                whileTap={{ scale: 0.95 }}
                style={{
                  fontFamily: FONTS.sans, fontSize: 12, fontWeight: 600,
                  padding: "6px 14px", borderRadius: 20, cursor: "pointer", transition: "background 0.15s, color 0.15s, border-color 0.15s",
        ...
        {/* Update cards — timeline cascade, re-animates on filter change */}
      ...
      <PageSection bg={COLORS.fieldMist}>
        ...
        <Stagger style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20, maxWidth: 900, margin: "0 auto" }}>
          {u.howItems.map((item) => (
            <StaggerItem key={item.title} style={{ height: "100%" }}>
              <motion.div whileHover={{ y: -4 }} transition={{ duration: MOTION.dur.fast, ease: EASE }} style={{ height: "100%" }}>
                <div style={{ background: "white", borderRadius: 12, padding: "20px", border: "1px solid #e5eee9", textAlign: "center", height: "100%" }}>
